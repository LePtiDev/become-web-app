    ...
    <transition name="snackbar">
      <div class="myc-snackbar" v-if="settings.status">
        <div class="snackbar-icon" :class="'snackbar-icon-' + settings.type">
          <v-icon v-if="settings.type === 'success'" size="24"
            >mdi-check-circle-outline</v-icon
          >
          <v-icon v-if="settings.type === 'error'" size="24"
        ...
        <div
          v-if="!settings.primaryFunction"
          class="snackbar-close"
          @click="closeSnasckbar()"
        >
